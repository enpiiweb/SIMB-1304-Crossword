...
%>
	...
	<tr>
		...
		<b> 
			<%if VarType(RS("DocTitle")) = 1 or RS("DocTitle") = "" then%>
				<a href="http:/<%=RS("vpath")%>"><%= Server.HTMLEncode( RS("filename") )%></a>
			<%else%>
				<a href="http:/<%=RS("vpath")%>"><%= Server.HTMLEncode(RS("DocTitle"))%></a>
			<%end if%>
		</b></td>
	</tr>
	...
	<tr>
		<td valign=top>
            ...
            <%end if%>
			<p>
			<i class="RecordStats"><a href="http:/<%=RS("vpath")%>" class="RecordStats" style="color:blue;">http:/<%=RS("vpath")%></a>
